<?php
	if(isset($_POST)){
		...
		if($query->num_rows > 0){	
			...
			if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'You need to timein first';
...
//break1in
				...
				$sql1 = "SELECT * FROM attendance WHERE employee_id = '$id' AND date = '$date_now' AND break1_in";
				 $query = $db->query($sql1);
					
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'You havent break1 out for today';
				 }
			...
			//breakout1
				...
				$sql1 = "SELECT * FROM attendance WHERE employee_id = '$id' AND date = '$date_now' AND break1_out";
				 $query = $db->query($sql1);
					
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'You havent break1 in for today';
				 }
			...
			//breakout2
				...
				if($query->num_rows < 1){
				 ...
				 $sql1 = "SELECT * FROM attendance WHERE employee_id = '$id' AND date = '$date_now' AND break2_out";
				 $query = $db->query($sql1);
					
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'You havent break2 in for today';
				 }
				else{
					$row = $query->fetch_assoc();
					...
					if(!empty($outnow) && !empty($srow['time_out'])){
						
						$outstatus = ($outnow > $srow['time_out']) ? 0 : 1; //bka nag execute dito ung code  dpat pag 
						...
						$sql = "UPDATE attendance SET time_out = NOW() , status_out = '$outstatus' WHERE employee_id = '$id'";
