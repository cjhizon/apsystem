  ...
  //$sql = "SELECT *, employees.employee_id AS empid, attendance.id AS attid FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id WHERE employees.employee_id=$empid AND date >= '$startdate'  and date <= '$enddate' $tmpEmpNum  ORDER BY attendance.date DESC, attendance.time_in DESC";
    ...
    ?>
      <form action="" method="POST">
            ...
            <input type="hidden" value="<?php echo  $empid = $_GET['employee_id'];?>" > </>
          ...
          <form action="viewagent.php?employee_id=<?php echo $empid;?>" method="POST">
            <input type="hidden" name="startdatepdf" value="<?php echo $startdatetext?>">
            <input type="hidden" name="enddatepdf" value="<?php echo $enddatetext?>">
            <input type="submit" class="btn btn-primary btn-user" value="PDF">
             <button onclick="doExport('#datainfo',{type:'excel'});" class="btn btn-primary btn-user" >CSV</button>
          </form>
         ...
         // $startdate= $row['date'];
              ...
              ?>   
               <td colspan="3" style=" font-size: 15px;">Employee ID: &emsp;<?php echo "".$empid.""; "".$startd."" ?></td>
