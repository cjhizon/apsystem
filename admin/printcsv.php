<?php
	...
	if(isset($_POST["export"])) {
		...
		fputcsv($output, array('Date', 'Time_in', 'Break1in ', 'Break1out','Break2in','Break2out','Timeout','Late', 'Undertime','Overtime', 'Total Hours', 'Total Break', 'Actual Hours'));
		$sql = mysqli_query($db,  " SELECT *, employees.employee_id AS empid, attendance.id FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id LEFT JOIN late ON late.emp_id=attendance.employee_id LEFT JOIN undertime ON undertime.emp_id=attendance.employee_id LEFT JOIN overtime ON overtime.employee_id=attendance.employee_id WHERE  date >= '$startdate'  and date <= '$enddate' $tmpEmpNum  ORDER BY attendance.date DESC, attendance.time_in DESC");       
