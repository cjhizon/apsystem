          ...
          <br>
            ...
            <div class="box-body">
              <table id="datainfo" class="table table-bordered table-striped table-hover js-exportable">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT *, undertime.ut_id AS utid, employees.employee_id AS empid FROM undertime LEFT JOIN employees ON employees.id=undertime.emp_id LEFT JOIN campaign ON campaign.id=employees.campaign_id LEFT JOIN batch ON batch.id=employees.batch_id WHERE date_ut >= '$startdate'  and date_ut <= '$enddate' ORDER BY  date_ut DESC";
                    $query = $db->query($sql);
                    while($row = $query->fetch_assoc()){
                        ?>
                        <tr>
                          <td class='hidden'></td>
                          <td><?php echo "".date('M d, Y', strtotime($row['date_ut']))."";  ?></td>
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      ...
      $('.utid').val(response.utid);
      $('#datepicker_edit').val(response.date_ut);
      $('#undertime_date').html(response.date_ut;
      $('#hours_edit').val(hour);
      $('#mins_edit').val(min);
